...
def img_upload = (RequestObject)findTestObject('Images_Upload1', [('token') : tokenacces, ('ktp') : ktp, ('id') : GlobalVariable.glb_dataid,
	('kk') : kk])

'Cek response data'
ResponseObject response_img = WS.sendRequest(img_upload)
...
'verify response status code'
WS.verifyResponseStatusCode(response_img, 200)

'show response to report in test suite'
String response_imgage = response_img.getResponseText()

'print log response text body'
KeywordUtil.logInfo("$response_imgage")
