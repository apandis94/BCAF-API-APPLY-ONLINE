...
submit_data = WS.sendRequest(findTestObject('Apply_Online', [('token') : tokenacces, ('ip_addres') : ip_addres
            ...
            , ('od_namamo') : od_namamo, ('od_namabm') : od_namabm, ('od_sumberapp') : od_sumberapp, ('od_idapp') : od_idapp
...
def response_in = slurper2.parseText(submit_data.getResponseBodyContent())

def noid = response_in.id

def massage = response_in.desc
...
'verify response status code'
WS.verifyResponseStatusCode(submit_data, 200)

'show response to report in test suite'
response_in = submit_data.getResponseText()

KeywordUtil.logInfo("$response_in")
